...
class ModelFactory():
    def __init__(self, datagen, net='VGG16', batch_size=16, lr=0.0001, dropout_keep_prob=0.8, model_dir='checkpoints', input_size=299, fine_tune=False, pretrained_path=None):
    ...
    def train(self, session):
        ...
        train_layer = tf.contrib.framework.get_variables('InceptionResnetV2/Logits', None, tf.GraphKeys.TRAINABLE_VARIABLES)
        train_layer += tf.contrib.framework.get_variables('InceptionResnetV2/Conv2d_7b_1x1', None, tf.GraphKeys.TRAINABLE_VARIABLES)
        train_layer += tf.contrib.framework.get_variables('InceptionResnetV2/Block8', None, tf.GraphKeys.TRAINABLE_VARIABLES)
        train_layer += tf.contrib.framework.get_variables('InceptionResnetV2/Mixed_7a', None, tf.GraphKeys.TRAINABLE_VARIABLES)
        train_layer += tf.contrib.framework.get_variables('InceptionResnetV2/Block17', None, tf.GraphKeys.TRAINABLE_VARIABLES)
        train_layer += tf.contrib.framework.get_variables('InceptionResnetV2/Mixed_6a', None, tf.GraphKeys.TRAINABLE_VARIABLES)
        train_layer += tf.contrib.framework.get_variables('InceptionResnetV2/Block35', None, tf.GraphKeys.TRAINABLE_VARIABLES)
        train_layer += tf.contrib.framework.get_variables('InceptionResnetV2/Mixed_5b', None, tf.GraphKeys.TRAINABLE_VARIABLES)
        ...
        generate_train_batch = self.datagen.generate_batch_train_samples(self.datagen.train_img_cnt_dict,
            self.datagen.trian_img_path_dict, batch_size=self.batch_size)
        total_loss = 0
        count = 0
        ...
        for step in xrange(last_step + 1, 100000000):
            ...
            if step != 0 and step % 2000 == 0:
                ...
                if N % self.batch_size != 0:
                    batches += 1
                validate_samples = self.datagen.generate_validate_samples(self.datagen.validate_img_cnt_dict,
                self.datagen.validate_img_path_dict, self.batch_size)
